...
_lr_productions = [
  ...
  ('exp -> term','exp',1,'p_exp','parser.py',81),
  ('exp -> term plus_minus exp','exp',3,'p_exp','parser.py',82),
  ('term -> factor','term',1,'p_term','parser.py',92),
  ('term -> factor multiply_divide term','term',3,'p_term','parser.py',93),
  ('factor -> OPEN_PARENTHESIS expression CLOSE_PARENTHESIS','factor',3,'p_factor','parser.py',103),
  ('factor -> plus_minus_factor var_cte','factor',2,'p_factor','parser.py',104),
  ('factor -> var_cte','factor',1,'p_factor','parser.py',105),
  ('var_cte -> ID','var_cte',1,'p_var_cte','parser.py',117),
  ('var_cte -> INT','var_cte',1,'p_var_cte','parser.py',118),
  ('var_cte -> FLOAT','var_cte',1,'p_var_cte','parser.py',119),
  ('plus_minus_factor -> PLUS','plus_minus_factor',1,'p_plus_minus_factor','parser.py',126),
  ('plus_minus_factor -> MINUS','plus_minus_factor',1,'p_plus_minus_factor','parser.py',127),
  ('plus_minus_factor -> empty','plus_minus_factor',1,'p_plus_minus_factor','parser.py',128),
  ('multiply_divide -> MULTIPLY','multiply_divide',1,'p_multiply_divide','parser.py',135),
  ('multiply_divide -> DIVIDE','multiply_divide',1,'p_multiply_divide','parser.py',136),
  ('plus_minus -> PLUS','plus_minus',1,'p_plus_minus','parser.py',144),
  ('plus_minus -> MINUS','plus_minus',1,'p_plus_minus','parser.py',145),
  ('condition -> IF OPEN_PARENTHESIS expression CLOSE_PARENTHESIS block else SEMICOLON','condition',7,'p_condition','parser.py',152),
  ('else -> ELSE block','else',2,'p_condition_else','parser.py',158),
  ('else -> empty','else',1,'p_condition_else','parser.py',159),
  ('expression -> exp','expression',1,'p_expression','parser.py',165),
  ('expression -> exp GREATER_THAN exp','expression',3,'p_expression','parser.py',166),
  ('expression -> exp GREATER_THAN_EQUAL exp','expression',3,'p_expression','parser.py',167),
  ('expression -> exp LESS_THAN exp','expression',3,'p_expression','parser.py',168),
  ('expression -> exp LESS_THAN_EQUAL exp','expression',3,'p_expression','parser.py',169),
  ('expression -> exp NOT_EQUAL exp','expression',3,'p_expression','parser.py',170),
  ('return -> RETURN_ID expression SEMICOLON','return',3,'p_return','parser.py',180),
  ('write -> PRINT OPEN_PARENTHESIS write_exp CLOSE_PARENTHESIS SEMICOLON','write',5,'p_write','parser.py',186),
  ('write_exp -> expression','write_exp',1,'p_write_exp','parser.py',192),
  ('write_exp -> expression COMMA write_exp','write_exp',3,'p_write_exp','parser.py',193),
  ('write_exp -> STRING','write_exp',1,'p_write_exp','parser.py',194),
  ('write_exp -> STRING COMMA write_exp','write_exp',3,'p_write_exp','parser.py',195),
  ('type -> INT_ID','type',1,'p_type','parser.py',202),
  ('type -> FLOAT_ID','type',1,'p_type','parser.py',203),
